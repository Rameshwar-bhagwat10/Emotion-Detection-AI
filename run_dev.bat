...
title Emotion Detection AI - Development Runner
...
echo ==============================================================================
echo        EMOTION DETECTION AI - DEVELOPMENT ENVIRONMENT (DEV MODE)
echo ==============================================================================
echo.
...
echo  - Frontend Web: Next.js Development Server with HMR (Port 3000)
echo ------------------------------------------------------------------------------
echo.

:: 3. Launch Backend in a dedicated titled window with auto-reload
start "Emotion AI - Backend (FastAPI Dev)" cmd /k "cd /d "%API_DIR%" && echo [*] Starting FastAPI with Auto-Reload on http://0.0.0.0:8000 ... && "%PYTHON_EXE%" -m uvicorn app.main:app --reload --reload-dir . --reload-dir ../../ml --host 0.0.0.0 --port 8000"

:: Wait 2 seconds before launching frontend
timeout /t 2 /nobreak >nul

:: 4. Launch Frontend in a dedicated titled window
start "Emotion AI - Frontend (Next.js Dev)" cmd /k "cd /d "%WEB_DIR%" && echo [*] Starting Next.js Dev Server on http://localhost:3000 ... && %PKG_MGR% dev"
